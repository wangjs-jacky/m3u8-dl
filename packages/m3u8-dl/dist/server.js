 ...
 * 递归扫描目录，查找所有临时分片目录
 */
function findAllTempDirs(rootDir, maxDepth = 2) {
    const result = [];
...
function scanInterruptedTasks() {
    ...
    for (const tempPath of tempDirPaths) {
        ...
        // 尝试加载元数据
        const meta = downloader_1.DecryptingDownloader.loadMeta(tempPath);
        // 检查是否有分片
        ...
        // 存储元数据路径用于恢复
        downloads[taskId]._metaPath = path.join(tempPath, 'task_meta.json');
        downloads[taskId]._tempPath = tempPath;
...
app.post('/api/download/start', (req, res) => {
    ...
    // 创建下载器
    const downloader = new downloader_1.DecryptingDownloader(downloadId, {
        url,
        outputPath,
        referer: referer || '',
...
app.post('/api/download/:id/preview', async (req, res) => {
    ...
    try {
        ...
        // 计算时长
        const duration = (0, merger_1.estimateDuration)(segmentFiles.length, 6);
        // 更新任务状态
        const previewFile = {
